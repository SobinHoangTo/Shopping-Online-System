                ...
                <div class="form-left">
                    ...
                    <select id="category" name="category" required>
                        <c:forEach var="category" items="${categories}">
                            <option value="${category.id}" ${product != null && category.id == product.categoryId ? 'selected' : ''}>${category.name}</option>
                        </c:forEach>
                    </select>
                    ...
                    <textarea id="description" name="description">${product != null ? product.description : ''}</textarea>
                </div>

                <div class="form-right">
                    <div class="image-box">
                        <img id="previewImage" src="${product != null && product.image != null ? product.image : 'https://via.placeholder.com/365'}" alt="Product Image">
                    </div>
                    <input type="file" name="productImages" id="productImages" accept="image/*" onchange="previewImage(event)">
                </div>

                <div class="buttons">
                    <button type="submit" name="action" value="${product != null ? 'update' : 'save'}">${product != null ? 'Save Changes' : 'Add Product'}</button>
